...
equipos_continuos = [
    "Refrigerador",
    "Maquina expendedora (Snacks)", 
    "Maquina expendedora (Bebidas frias)",
...
potencias_nominales = {
    ...
    "Incandescente": 60, "Halógena": 50, "Computadora de escritorio": 250, "Laptop": 20, "Enfriador/calentador de agua": 600,
    "Refrigerador": 47, "Cafetera": 1000, "Microondas": 1000, "Parrilla o estufa eléctrica": 1500, "Maquina expendedora (Snacks)": 150, 
    "Maquina expendedora (Bebidas frias)": 450, "Horno eléctrico": 1200, "Purificador de aire": 150,
...
consejos = {
    ...
    "Ventilador de techo": [
        "Refresca naturally antes de encenderlo.",
    ...
    "Hidroneumático": [
        ...
        "Se sugiere programar su uso en horarios con menor demanda de corriente. (horas valle)" 
    ],
    "Compresor": [
...
def calcular_kwh_mes(potencia_w, num_equipos=1, horas=8, factor_mensual=21, continuo=False):
    if continuo:
        kwh = (potencia_w / 1000) * num_equipos * 24 * 30
    else:
...
with tab_residencial:
    ...
    # 1. Se agrega "Equipos de fuerza" a la lista de opciones
    usos_residenciales = [
        "Iluminación", "Acondicionamiento de aire residencial", "Electrodomésticos residenciales", 
        "Equipos de cómputo", "Entretenimiento", "Equipos sanitarios", "Equipos de fuerza", "Otros"
    ]
                ...
                for sub in subusos[uso]:
                    ...
                    if marcado:
                        ...
                        if uso == "Acondicionamiento de aire residencial" and sub in cop__data:
                            ...
                            eficiencia = 0.75 if sub in ["Hidroneumático", "Bomba de agua"] else 0.85
                            potencia_w = (hp * 746) / eficiencia
                    ...
                    if uso in subusos:
                        for sub in subusos[uso]:
                            ...
                            if marcado:
                                ...
                                # 2. Bloque agregado para Equipos de fuerza en modo "Por piso"
                                elif uso == "Equipos de fuerza":
                                    num_equipos = st.number_input(f"N° equipos ({sub})", min_value=1, step=1, key=f"{key_base}_num")
                                    hp = st.number_input(f"Potencia en Caballos de Fuerza (HP) ({sub}):", min_value=0.1, value=1.0, step=0.5, key=f"{key_base}_hp")
                                    horas = st.number_input("Horas/día:", min_value=0.1, max_value=24.0, value=8.0, step=1.0, key=f"{key_base}_hr")
